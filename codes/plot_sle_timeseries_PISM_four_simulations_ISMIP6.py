...
if __name__ == "__main__":
    ...
    ### Read Sea level rise from the PISM experiments (all experiments)
    ...
    for i, model in enumerate(models):
        ...
        path='/Users/home/siewpe/codes/greenland_emulator/save_pism_sle_timeseries/%s/'%foldernames[model]
    ...
    ###
    ### Start plotting the the four PISM simulations up to 2600, including IMBIE (Figure S6)
    plot_imbie=True
    ...
    ## Selected PISM experiment
    pism_model='MIROC5_rcp85_pad2600_8var'; forcing="RCP85"; exp='exp05' # MIROC5 RCP85
    pism_model='MIROC5_rcp26_pad2600_8var'; forcing="RCP26"; exp='exp07' # MIROC5 RCP26
    ...
    ismip6_path="/mnt/data/data_b/Greenland_ISMIP6_2100_2300_scalar/ISMIP6_sea_level_rise_scalar/v7_CMIP5_pub/"
    ismip_slrs=[]
    for model in ismip6_models :
        institute,version=model.split('-')
        data=xr.open_dataset('%s/%s/%s/%s_05/scalars_mm_cr_GIS_%s_%s_%s.nc'%(ismip6_path,institute,version,exp,institute,version,exp))
        data=data['sle'].values*-1*100
        data=xr.DataArray(data,dims=['time'],coords={'time':range(2015,2101)})
        ## Read the control
        data_control=xr.open_dataset('%s/%s/%s/ctrl_proj_05/scalars_mm_GIS_%s_%s_ctrl_proj.nc'%(ismip6_path,institute,version,institute,version)) 
        ...
        #ismip_slrs.append(data+data_control) # Add control back 
    ...
    for model in range(ismip_slrs.models.size):
        ...
        ys.append(y)
    ax1.plot(ismip_slrs.time,np.array(ys).mean(axis=0),color='blue',zorder=3,lw=2,label='ISMIP6 models forced by MIROC5 %s (%s)'%(forcing,exp))
    param_no=slr_datas[pism_model].params.size
    ys=[]
    for en in range(param_no):
        y=slr_datas[pism_model].isel(params=en).sel(time=slice(2015,2100))
        ax1.plot(range(2015,2101),y,color='red',alpha=0.5,zorder=1,lw=0.5)
        ys.append(y)
    ax1.plot(range(2015,2101),np.array(ys).mean(axis=0),color='red',zorder=2.1,lw=2,label='PISM forced by MIROC5 %s'%forcing)
