...
if __name__ == "__main__":
    ...
    if True:  ## to create weighting to selected parameter ensemble (this part is default)
        #predict_models=['BESTnew']; predict_years=range(1992,2021) # 
        predict_models=['BEST']; predict_years=range(1992,2021) # This BEST is better than the BESTnew
        predict_models_ens=['1']
        plot_imbie=True; calculate_and_save_weights=True
        colors=['gray']
        if False:  ## Make the train_years and predict_years consistent
    ...
    else: # For testing on simulations
    ...
    ## Predict prameters 
    predict_param_fn='parameters_LHC_1000_8param_range' # new range using 8 parameters (default)
    predict_param_fn='parameters_LHC_1000_8param_range_ISMIP' # Shift to ismip6-mean
    ...
    if plot_imbie: 
        ...
        if calculate_and_save_weights: # Calculate the difference between obs and each ensemble member (according to previous works)
            ...
            predict_model=[*yss][0]
            diff=yss[predict_model]-ice_loss.values
            std_obs=uncertainty.values
            #std_model=std_obs*10 
            # larger this number the weights of the max are smaller (more evenly distributed - won't give too much weights to a single ensemble)
            model_multiple=40
            model_multiple=30
            model_multiple=10
            ...
            for i in sort_idx[0:200]: # smallest weights
                ax1.plot(predict_years,yss[predict_model][i],color='royalblue',lw=1,zorder=10)
            for i in sort_idx[-200:]: # largest weights
                ax1.plot(predict_years,yss[predict_model][i],color='red',lw=1,zorder=10)
    ...
    ### Save the figure
    fig_name = 'historical_emulation_1992_2020'
