...
if __name__ == "__main__":
    ...
    if True: ### Create the timeseries
        ...
        path="../CAT_timeseries_Climate_Analytics/"
        if True: # Default
            scenarios={'current':['CPP_low','CPP_high'],
                        'target':['Targets_2030_and_2035'],
                        'pledge':['Pledges_and_Targets','Pledges_low'],
                        'optim':['Optimistic_scenario']}; save_folder='policy_full600'
        else: # the new method (not used anymore)
            scenarios={'current_low':['CPP_low'],
                       'current_high':['CPP_high'],
                       'target_low':['Targets_2030_and_2035'],
                       'target_high':['Targets_2030_only'],
                       'pledge_low':['Pledges_low'],
                       'pledge_high':['Pledges_and_Targets'],
                       'optim_low':['Optimistic_low'],
                       'optim_high':['Optimistic_scenario']}; save_folder='policy_full1200'
        ...
        # np.median(data_tss['target_high'],axis=0)[350] # this will equal to the GMT timeseries published online for year 2100
        #ipdb.set_trace()
        ...
        for scenario in scenarios:
            ...
            for j, gmt_ts in enumerate(scen_tss): 
                ...
                ## Calculate the linear trends from 2080 to 2100 (we call it no-pad data)
                ...
                ## Create them for saving (here we only save the no-pad data)
                ...
                gmt_tss=[gmt_ts_nopad] # both from 1850 to 2300
                pad_saves=['nopad2300']
                for i, pad_save in enumerate(pad_saves):
                    ...
                    ## Save the data
                    save_data=np.column_stack((gmt_years.values,gmt_data.values))
                    save_name='distno'+str(j)+'_'+scenario+'_'+pad_save
                    np.save('../save_gmt/tas_ts_create/%s/gmt_%s_%s_en%s_%s.npy'
                                                                %(save_folder,var,save_name,en,region),np.float64(save_data))
    ...
    if True: ### Plotting the data after creating the timeseries
        ...
        if True: ## Plot Berkerley GMST observations (we should change to use GSAT instead of GMST - 20th v3 reanalysis + ERA5)
            ...
            gmt_read=np.load('../save_gmt/tas_ts_create/BEST/gmt_%s_BEST_en%s_%s.npy'%('tas','1','global'))
        ...
        ## Plot CAT
        #pad_saves=['pad2300','nopad2300']
        pad_saves=['nopad2300','nopad2300'] # only plot the no-pad data (linear extrapolation)
        ...
        if True:
            ...
            cat_colors={'current_low':'red','current_high':'red',
                        'target_low':'orange','target_high':'orange',
                        'pledge_low':'yellow','pledge_high':'yellow',
                        'optim_low':'green','optim_high':'green'}
            cat_names=list(scenarios.keys())
            cat_numbers={scen:600 for scen in cat_names}
        ...
        for i, pad_save in enumerate(pad_saves):
            for cat in cat_names:
                ...
                for no in range(cat_numbers[cat]):
                    ...
                    #ipdb.set_trace()
                    gmt_read=np.load('../save_gmt/tas_ts_create/%s/gmt_%s_%s_en%s_%s.npy'
                                                                                            %(save_folder,'tas',save_name,'1','global'))
                    gmt_ts=gmt_read[:,1]
