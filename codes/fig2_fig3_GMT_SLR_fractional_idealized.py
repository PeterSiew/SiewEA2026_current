...
if __name__ == "__main__":
    ...
    predict_param_fn='parameters_LHC_1000_8param_range_large' 
    ...
    ### Get all predicting models for RCP and CATs scenaris up to 2300
    ### Read all extended RCPs
    rcps=['rcp26','rcp45','rcp60','rcp85']
    folder='../save_gmt/tas_ts_create/CMIP5/'
    file_list=os.listdir(folder)
    file_selects=[]
    models=[]
    ens=[]
    for file in file_list:
        if ('rcp' in file) & ('_nopad2300_' in file) & ('CMIP5mean' not in file): # Why not deleting those CMIP5mean files?
    ...
    if True:
        ...
        cats_full_dist={'optim':600,'pledge':1200,'target':600,'current':1200} # full
        cats_random_draw={'optim':300,'pledge':300,'target':300,'current':300} # default in the paper
    else: # This is not used in the current stage
        cats=['optim_low','optim_high','pledge_low','pledge_high','target_low','target_high','current_low','current_high']
        cats_full_dist={cat:600 for cat in cats}
        #cats_random_draw={cat:100 for cat in cats}
        cats_random_draw={cat:300 for cat in cats}
    ...
    print("Read training data-Finish")
    ...
    if plotting_fig2_gmt:
        ...
        reload(fig23);fig23.plotting_figure2_gmt(predict_models_rcps,predict_models_cats,models_colors,rcp_models_label,cat_models_label,gmt_datas)
    ...
    for model in predict_models_new:
        argu=(model,predict_params,predict_years,gmt_datas[model],None,X_mean,X_std,Y_mean,Y_std,regress)
        argus.append(argu)
    ...
    # no. of models * no. of parameters * years
    Y_predict_reshape=np.array(Y_predict_all).reshape(len(predict_models_new),predict_params.shape[0],len(predict_years)) 
    print("Start prediction-Finish")


    ### Separate the resulting data into groups (4 RCPs and 4 CATs)
    ### This part should also go to multi-processing to save time
    ## the weight used to define prior and posteria
    if False: ## Recreate the cats group - to group low & high into the same group
        cats=['optim_low','optim_high','pledge_low','pledge_high','target_low','target_high','current_low','current_high']
        Y_predict_groups_cats={cat:[] for cat in cats}
        cats=['optim','pledge','target','current']
    rcps_cats=rcps+cats
    ...
    for i, model in enumerate(predict_models):
        group1=model.split('_')[1]
        Y_predict_groups[group1].append(Y_predict_reshape[i])
        ## Leave a seperate dictionary to keep low and high for CAT groups
        #if group1 in cats:
        #    group2=model.split('_')[2]
        #    Y_predict_groups_cats[group1+'_'+group2].append(Y_predict_reshape[i])
    ## To further reshape the array
    ...
    if save_Y_predict: ### Save the Y_predict_file (prior - no weight adjustment) for testing the posteria for different mm
        ...
        np.save("../save_Y_predicts/Y_predict_fig3",Y_predict_groups) 
        print("Saving Done")
    ## To combine the low and high scenario for CAT
    if False:
        Y_predict_groups_cats_np={}
        for cat in cats:
            Y_predict_groups_cats_np[cat]=np.stack([Y_predict_groups_cats[cat+'_low'],Y_predict_groups_cats[cat+'_high']],axis=0)
    ...
    for group in rcps_cats: 
        ...
        slr_post_qs[group][0.05]=np.percentile(Y_predict_groups_new[group],5,axis=0,weights=Y_predict_weights[group],method="inverted_cdf")
        slr_post_qs[group][0.95]=np.percentile(Y_predict_groups_new[group],95,axis=0,weights=Y_predict_weights[group],method="inverted_cdf")
    
    ### For tamsin edward's report
    save_post_slr=True
    save_post_slr=False
    if save_post_slr: 
        save_slr=np.array([slr_post_qs[group][q] for group in rcps_cats for q in [0.05,0.5,0.95]]).T
        save_data=np.column_stack([predict_years,save_slr])
        rcps_cats_name=[group+'_q'+str(int(q*100)) for group in rcps_cats for q in [0.05,0.5,0.95]]
        header_cols = ["Year"]+rcps_cats_name
        header_str = ",".join(header_cols)
        fmt_list = ["%d"] + ["%.1f"] * (save_data.shape[1] - 1)
        np.savetxt("./sea_level_proejctions_SiewEA2026.txt", save_data, fmt=fmt_list, delimiter=",", header=header_str, comments="")
    ...
    if True & plotting_fig3:
        ...
        ## Uppper panel (ax1 and ax2); for RCPS projection (ax1); for CATs projection (ax2)
        max_q=0.83; min_q=0.17
        ## For RCP sencerio (ax1)
        ...
        ## For RCPs and CATs (With history matching)
        scen_vars={}; gcm_vars={}; param_vars={}; param_vars_nohist={}
        global_vars={}; residual_vars={}
        rcp_or_cat=['rcp','cat']
        rcps_and_cats=[rcps,cats]
        for i, scens in enumerate(rcps_and_cats):
            save_dict=rcp_or_cat[i]
            ## 1) Scnearios (RCP/CAT) Variance- i) mean across parameters (weighted), ii) mean across GCM in each RCP iii) variance across RCPs
            #scen_var_backup=np.array([np.average(Y_predict_groups[scen].mean(axis=0),weights=weights,axis=0) for scen in scens]).var(axis=0)
            scen_var=np.array([np.average(Y_predict_groups[scen],weights=weights,axis=1).mean(axis=0) for scen in scens]).var(axis=0)
            ## 2) GCM Variances- i) mean across parameters (weighted), ii) variance across GCMs, iii) mean across RCPS
            gcm_var=np.array([np.average(Y_predict_groups[scen],weights=weights,axis=1).var(axis=0) for scen in scens]).mean(axis=0)
            #gcm_var_backup=np.array([np.average(Y_predict_groups[scen].var(axis=0),weights=weights,axis=0) for scen in scens]).mean(axis=0)
            ## This is wrong. As it calculates the standard devidation, average, and the ^2 to be the variance
            #gcm_var=np.array([np.average(Y_predict_groups[scen].std(axis=0),weights=weights,axis=0) for scen in scens]).mean(axis=0)**2
            ## 3) Param Variances- i) variance across parameters (weighted), ii) mean across GCMs), iii) mean across RCPS
            if True: # Old method (see above)
                param_var=[]
                for scen in scens:
                    values=Y_predict_groups[scen]
                    values_mean=np.average(values,weights=weights,axis=1)
                    values_mean_expand=values_mean[:,np.newaxis,:]
                    values_mean_final=np.tile(values_mean_expand,(1,values.shape[1],1))
                    values_var=np.average((values-values_mean_final)**2,weights=weights,axis=1)
                    ## mean across GCMs
                    param_var.append(values_var.mean(axis=0))
                ## Mean across RCPs
                param_var=np.array(param_var).mean(axis=0)
                ## The parameter variance without history matching (for the grey shading in middle panel)
                param_var_nohist=np.array([Y_predict_groups[scen].var(axis=1).mean(axis=0) for scen in scens]).mean(axis=0)
            else: # new method- i) Mean across models; mean across rcps; variance across parameters
                values=np.array([Y_predict_groups[scen].mean(axis=0) for scen in scens]).mean(axis=0)
                values_mean=np.average(values,weights=weights,axis=0)
                param_var=np.average((values-values_mean)**2,weights=weights,axis=0)
                param_var_nohist=np.array([Y_predict_groups[scen].mean(axis=0) for scen in scens]).mean(axis=0).var(axis=0)
            ## Get the Global,True variance
            #data_flat=[]; gweight=[] #global weights
            datas=[]; gws=[]; lws=[]
            for scen in scens:
                data=Y_predict_groups[scen]
                datas.append(data)
                gw=(1/len(scens))*(1/data.shape[0])*(1/data.shape[1])
                gw_array=np.full((data.shape[0], data.shape[1]), gw)
                gws.append(gw_array)
                lw_array=np.full((data.shape[0],data.shape[1]),weights)
                lws.append(lw_array)
            ## To aggregate global-data
            datas=np.vstack(datas)
            data_flat=datas.reshape(-1,datas.shape[-1])
            ## For global-weights
            gws=np.vstack(gws)
            gws_flat=gws.reshape(-1)
            ## For local-weights (parameter-weights)
            lws=np.vstack(lws)
            lws_flat=lws.reshape(-1)
            ## Combine the two weights
            lws_flat=lws_flat/lws_flat.sum() # Normalize the parameter weight across global samples
            cws_flat=gws_flat*lws_flat # cws mean combined weights
            cws_flat=cws_flat/cws_flat.sum() # Noralize after multiplication
            ## Calulate the variance
            data_flat_mean=np.average(data_flat,weights=cws_flat,axis=0)
            data_flat_var=np.average((data_flat-data_flat_mean)**2,weights=cws_flat,axis=0)
            #data_flat_var=(((data_flat-data_flat_mean)**2)*cws_flat[:,np.newaxis]).sum(axis=0)
            ## Save them into dictionary
            scen_vars[save_dict]=scen_var
            gcm_vars[save_dict]=gcm_var
            param_vars[save_dict]=param_var
            param_vars_nohist[save_dict]=param_var_nohist
            global_vars[save_dict]=data_flat_var
            residual_vars[save_dict]=data_flat_var-scen_var-gcm_var-param_var # We combine that residual var is zero
        ### Finish Looping RCPs and CATs
        if True: # Set the colobar for parameter variance (with history-matching)
            green_color='forestgreen'
        else: # plotting the supp - fractioanl variance without history matching (gray instead of green)
            param_vars=param_vars_nohist
            green_color='lightgray'
        ### Calculate the variances and realtive variance of each component
        ## For RCPs
        rcps_total_var=scen_vars['rcp']+gcm_vars['rcp']+param_vars['rcp']
        #rcps_total_var=global_vars['rcp']
        rcps_scenario_frac=(scen_vars['rcp']/rcps_total_var)*100
        rcps_param_frac=(param_vars['rcp']/rcps_total_var)*100
        rcps_gcm_frac=(gcm_vars['rcp']/rcps_total_var)*100
        ## For CATs
        cats_total_var=scen_vars['cat'] + gcm_vars['cat'] + param_vars['cat']
        cats_scenario_frac=(scen_vars['cat']/cats_total_var)*100
        cats_param_frac=(param_vars['cat']/cats_total_var)*100
        cats_gcm_frac=(gcm_vars['cat']/cats_total_var)*100
        ###
        ### Start the figure
        ...
        ## RCPS
        gcm_1=(gcm_vars['rcp'])[yr_idx]
        scenario_1=(scen_vars['rcp'])[yr_idx]
        param_1=(param_vars['rcp'])[yr_idx]
        param_1_no_histmatch=(param_vars_nohist['rcp'])[yr_idx]
        ## CATS
        gcm_2=np.array(gcm_vars['cat'])[yr_idx]
        scenario_2=np.array(scen_vars['cat'])[yr_idx]
        param_2=np.array(param_vars['cat'])[yr_idx]
        param_2_no_histmatch=np.array(param_vars_nohist['cat'])[yr_idx]
        ...
        ax3.bar(x-x_adj*0.5,param_1,bar_width,bottom=gcm_1+scenario_1,edgecolor='k',hatch=hatch,color=green_color,lw=0,
                                                                    label='Ice-sheet model parameters\n(with history-calibration)')
        ax3.bar(x-x_adj*0.5,param_1_no_histmatch,bar_width,bottom=gcm_1+scenario_1+param_1,edgecolor='k',
                                                hatch=hatch,color='lightgray',lw=0,label='Ice-sheet model parameters\n(without history-calibration)')
        ## For CATs (plotting ax4)
        ax4.bar(x+x_adj*0.5,gcm_2,bar_width,bottom=0,color='royalblue',edgecolor='k',hatch=hatch,lw=0,label='MAGICC&CAT')
        ax4.bar(x+x_adj*0.5,scenario_2,bar_width,bottom=gcm_2,color='orange',edgecolor='k',hatch=hatch,lw=0,label='Policy actions')
        ax4.bar(x+x_adj*0.5,param_2,bar_width,bottom=gcm_2+scenario_2,color=green_color,edgecolor='k',hatch=hatch,lw=0,
                                                                                label='Ice-sheet model parameters\n(with history-calibration)')
        ax4.bar(x+x_adj*0.5,param_2_no_histmatch,bar_width,bottom=gcm_2+scenario_2+param_2,edgecolor='k',hatch=hatch,color='lightgray',
                                                                                lw=0,label='Ice-sheet model parameters\n(without history-calibration)')
        ### Set ax3/ax4 axis
        ...
        ###
        ### the lowest panel - (ax5 and ax6), fractional uncertainty
        ###
        ## RCPs: ax5
        ## Plot GCM fraction
        ...
        base=0; top=0
        ## Plot GCM fraction
        base=top; top=base+cats_gcm_frac
        ax6.fill_between(predict_years, base, top, fc='royalblue',color='royalblue',label='MAGICC&CAT')
        ...
        plt.savefig('../../graphs/%s_%s.png'%(dt.date.today(), fig_name), bbox_inches='tight', dpi=500, pad_inches=0.01)
    ...
    if print_everything:
        ...
        if True: # Print the fractional uncertainty of RCP scnearios in years 2050, 2100 and 2300
            ...
            yr_idx=[predict_years.index(yr) for yr in years_sel]
            print('RCP scenario fractional uncertainty in 2050 is: ',rcps_scenario_frac[yr_idx[0]])
            print('RCP scenario fractional uncertainty in 2100 is: ',rcps_scenario_frac[yr_idx[1]])
            print('RCP scenario fractional uncertainty in 2300 is: ',rcps_scenario_frac[yr_idx[2]])
            print("")
...
def plotting_figure2_gmt(predict_models_rcps,predict_models_cats,models_colors,rcp_models_label,cat_models_label,gmt_datas):
    ...
    for i, cat in enumerate(cats):
        ...
        if cat in ['optim','current']:
            ...
            print("The averaged GMT in years 2100 and 2300 for CAT %s are"%cat)
    ...
    if False: ## Plot Berkerley GMT observations
        ...
        save_name='BEST'
        gmt_read=np.load('../save_gmt/tas_ts_create/gmt_%s_%s_en%s_%s.npy'%('tas',save_name,'1','global'))
