...
def read_gmts(gmt_models,gmt_ens):
    ...
    for i, model in enumerate(gmt_models):
        ...
        elif model=='MIROC5_rcp85cooling_pad2600_8var':
        ...
        elif model=='MIROC5_rcp85_pad2600_8var':
            model_read='MIROC5_rcp85_pad2600'
            gmt_read=np.load('../save_gmt/tas_ts_create/CMIP5/gmt_%s_%s_en%s_%s.npy'%(tas_or_tos,model_read,gmt_en,region))
        elif model=='MIROC5_rcp26_pad2600_8var':
            model_read='MIROC5_rcp26_pad2600'
            gmt_read=np.load('../save_gmt/tas_ts_create/CMIP5/gmt_%s_%s_en%s_%s.npy'%(tas_or_tos,model_read,gmt_en,region))
        ## Predict models (just a different directoary)
        elif 'distno' in model: # just a different directory
            gmt_read=np.load('../save_gmt/tas_ts_create/policy_full600/gmt_%s_%s_en%s_%s.npy'%(tas_or_tos,model,gmt_en,region))
        elif 'stable2' in model: # just a different directory
            gmt_read=np.load('../save_gmt/tas_ts_create/policy_current_stable_yr3000/gmt_%s_%s_en%s_%s.npy'
                                                                                                %(tas_or_tos,model,gmt_en,region))
        elif "rcp" in model:
            model_read=model
            gmt_read=np.load('../save_gmt/tas_ts_create/CMIP5/gmt_%s_%s_en%s_%s.npy'%(tas_or_tos,model_read,gmt_en,region))
        elif "ssp" in model:
            model_read=model
            gmt_read=np.load('../save_gmt/tas_ts_create/CMIP6/gmt_%s_%s_en%s_%s.npy'%(tas_or_tos,model_read,gmt_en,region))
        elif "extra2300" in model:
            model_read=model
            gmt_read=np.load('../save_gmt/tas_ts_create/policy_stable_damped_overshoot/gmt_%s_%s_en%s_%s.npy'%(tas_or_tos,model_read,gmt_en,region))
        elif model=='BEST': 
            model_read=model
            gmt_read=np.load('../save_gmt/tas_ts_create/BEST/gmt_%s_%s_en%s_%s.npy'%(tas_or_tos,model_read,gmt_en,region))
        elif model=='BESTnew': 
            model_read=model
            gmt_read=np.load('../save_gmt/tas_ts_create/BESTnew/gmt_%s_%s_en%s_%s.npy'%(tas_or_tos,model_read,gmt_en,region))
        else:
            raise ValueError("Check this line - no model is correctly selected")
    ...
    return gmt_datas, None, None, None
...
def read_training_XY(train_models,train_years,gmt_datas,gmt_cum_datas,time_since_last_change,slr_relative_yr=2015):
    ...
    filenames={'ctrlProj':'1990-2100-ctrlProj',
            ...
            'NorESM1-M_rcp85_pad2300':'1990-2300-NorESM_85',
            'HadGEM2-ES_rcp85_pad':'1990-2300-HadGEM2_85',
            ## New simulation
    ...
    foldernames={'ctrlProj':'ctrlProj',
            ...
            'NorESM1-M_rcp85_pad2300':'NorESM_85',
            'HadGEM2-ES_rcp85_pad':'HadGEM2_85',
            ## New simulation
    ...
    models_years={
                ...
                'HadGEM2-ES_rcp85_pad':range(1985,2301),'NorESM1-M_rcp85_pad2300':range(1985,2301),
    ...
    for i, model in enumerate(train_models):
        ...
        path="../save_pism_sle_timeseries/%s/"%foldernames[model]
        ...
        for fn in file_list:
            ...
            data=xr.open_dataset(path+fn,decode_times=time_coder)['sea_level_rise_potential']
...
def do_prediction(model,predict_params,predict_years,gmt_data,gmt_cum_data,X_mean,X_std,Y_mean,Y_std,regress):
    ...
    ### This one is faster than the old as as it does the prediction for 1,000 parameters all together
