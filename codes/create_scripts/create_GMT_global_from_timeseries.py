...
import create_timeseries_greenland as ct
import tools_greenland as tools
...
if __name__ == "__main__":
    ...
    CMIPv='CMIP5'
    CMIPv='CMIP6'
    ## Remember to put "create_pad_till_2600=True" for CMIP5 (only creaing the pad GMT for MIROC5_26 and MIROC5_85 for training)
    #### Remember to select there
    create_pad_till_2600=True # Only for MICRO5 in CMIP5
    create_pad_till_2600=False # For CMIP6 - we don't need to pad until 2600

    #folder='/Users/home/siewpe/codes/greenland_emulator/save_gmt/gmt_ts_download/%s/'%CMIPv
    folder='../CMIP5_CMIP6_gmt_download/%s/'%CMIPv
    ...
    if CMIPv=='CMIP5':
        ...
        for file in file_list_new:
            model=file.split('_')[3] 
            ssp=file.split('_')[4] 
            models.append(model+'_'+ssp)
            #en=file.split('_')[5].replace('r','').replace('i1p1.dat','') 
            en='0' # this is the first ensemble
            ensembles.append(en)
    ...
    if create_pad_till_2600: # For the MIROC5 training simulation that extends up to 2600
    ...
    for i, model in enumerate(models):
        ...
        ## For pad till 2600, we only need MIROC5
        if create_pad_till_2600==True:
            if (model=="MIROC5_rcp26") | (model=="MIROC5_rcp85"):
                pass
            else:
                continue
        ...
        if CMIPv=='CMIP5':
            ...
            path=folder+'global_tas_mon_'+model+'_%s.dat'%en.zfill(3)
        ...
        if create_pad_till_2600:
            pad_saves=['pad2600','nopad2600'] # nopad2600 basically won't be save, as gmt_nopad is always None (no GCM simulations up to 2300)
        for j, gmt_ts in enumerate([gmt_pad, gmt_nopad]):
            ...
            np.save('../save_gmt/tas_ts_create/%s/gmt_%s_%s_%s_en%s_%s.npy'%(CMIPv,var,model,pad_saves[j],en,region),save_data)
    ...
    if False:
        ...
        if CMIPv=='CMIP5':
            ...
            rcp_colors={'ssp585':'red','ssp370':'tomato','ssp245':'orange','ssp126':'green'}
        ##!! go via the models with pad first, and then model with nopad (for nopad2600 - it doesn't work here as there is no nopad2600 model)
        for i, models_ens in enumerate([models_ens_pad, models_ens_nopad]): 
            #ipdb.set_trace()
            models=np.array(models_ens)[:,0]; ens=np.array(models_ens)[:,1]
            tss={}
            for j, model in enumerate(models):
                en=ens[j]
                gmt_read=np.load('../save_gmt/tas_ts_create/%s/gmt_%s_%s_%s_en%s_%s.npy'%(CMIPv,var,model,pad_saves[i],en,region))
            ...
            for rcp in sorted(tss.keys()):
                gmt_mean=xr.concat(tss[rcp],dim='rcp',join='outer').mean(dim='rcp')
                ...
                #save_data=np.column_stack((years.values,gmt_data.values,gmt_cum_data.values,X8,gmt_cum_data_gradient.values))
                save_data=np.column_stack((years.values,gmt_data.values))
                var='tas'; modell='%smean_%s'%(CMIPv,rcp); en='1'; region='global'# the mean of all models and ensembles
                np.save('../save_gmt/tas_ts_create/%s/gmt_%s_%s_%s_en%s_%s.npy'%(CMIPv,var,modell,pad_saves[i],en,region),save_data)
                print("Create %smean for %s %s %s %s %s"%(CMIPv,var,modell,pad_saves[i],en,region))
